...
lsts_set_root "$BATS_FILE_DIR"
lsts_set_langId "plaintext"

_script=""
_fixture=""
_write_script() { _script="$(mktemp /tmp/fake-ls-script.XXXXXX.sh)"; cat > "$_script"; }
_start_with_script() {
...
_make_fixture() {
    local f
    f="$(mktemp "$FIXTURES_DIR/fixture.XXXXXX.json")"
    printf '%s\n' "$1" > "$f"
    _fixture="$f"
    echo "$f"
}
...
teardown() {
    ...
    if [[ -n "$_fixture" ]]; then rm -f "$_fixture"; fi
}
...
EOF
    _start_with_script
    local fixture
    fixture="$(_make_fixture '{"jsonrpc":"2.0","id":2,"result":{"contents":{"kind":"plaintext","value":"hello"}}}')"
    lsts_hover "lsts_tests.bats:1:1" "$fixture"
}
...
EOF
    ...
    local fixture
    fixture="$(_make_fixture '{"jsonrpc":"2.0","id":2,"result":{"contents":{"kind":"plaintext","value":"hello"}}}')"
    run lsts_hover "lsts_tests.bats:1:1" "$fixture"
    [[ "$status" -ne 0 ]]
}
...
EOF
    _start_with_script
    local fixture
    fixture="$(_make_fixture '{"jsonrpc":"2.0","id":2,"result":[{"uri":"file://$GOROOT/src/fmt/print.go"}]}')"
    lsts_add_filter \
        "file:///nix/store/[a-z0-9]*-go-[0-9.]*/share/go" \
        'file://$GOROOT'
    lsts_definition "lsts_tests.bats:1:1" "$fixture"
}
